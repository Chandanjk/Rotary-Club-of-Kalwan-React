...
const Posts = (props) => {
          ...
          import(`../../images/upload_posts/${image}`)
            .then((image) => {
              ...
              foundIndex !== -1
                ...
                : console.log("");
              images_temp = [...images_temp, { _id: post._id, images: temp }];
  ...
  const createHandler = (event) => {
    ...
    const data = {
      _id: null,
      images: stateData.images.map(file => `${prefix}${file.name}`),
    ...
    const images_data = stateData.images.map(file => (new File([file],`${prefix}${file.name}`)))
    ...
    axios.post(process.env.API_URL + "/posts_data/postimage", formData).then((res) => {
      console.log(res);
    });
  ...
  const findImage = (_id) => {
    image = images.find((element) => element._id === _id) || {
      _id: "",
      images: [],
    };
  };
                    ...
                    >
                      <div className="post-entry-1 h-100 post-entry-design">
                        <div className="post-entry-image">
                          {images.length !== 0 ? findImage(post._id) : <></>}
